// Canonical source: bridges/log/guest.go — the log bridge's Go guest binding.
// Synced into rusm-go (packages/rusm-go/log.go) by `make sync-bridges`; edit this
...
import (
	...
	hostlog "github.com/archan937/rusm/packages/rusm-go/internal/wit/rusm/runtime/log"
)
...
//
// The generated `log` interface package is aliased `hostlog` so it doesn't clash with
// the standard library's `log` package this adapter routes.
func initLogging() {
...
func (h *hostHandler) Handle(_ context.Context, r slog.Record) error {
	...
	hostlog.Log(toLevel(r.Level), b.String())
	return nil
}
...
func toLevel(l slog.Level) hostlog.LogLevel {
	switch {
	case l >= slog.LevelError:
		return hostlog.LogLevelError
	case l >= slog.LevelWarn:
		return hostlog.LogLevelWarn
	case l >= slog.LevelInfo:
		return hostlog.LogLevelInfo
	default:
		return hostlog.LogLevelDebug
	}
}
...
func (logWriter) Write(p []byte) (int, error) {
	hostlog.Log(hostlog.LogLevelInfo, strings.TrimRight(string(p), "\n"))
	return len(p), nil
}
...
func Log(level slog.Level, message string) { hostlog.Log(toLevel(level), message) }
