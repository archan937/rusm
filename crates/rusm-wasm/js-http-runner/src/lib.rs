...
/// Per-app custom-bridge glue: empty in the prebuilt runner, overwritten by `rusm build` with
/// the app's bridges (the js-runner has its own copy with the identical shape — both are
/// regenerated by the one `gen_runner_bridges_gen`, so the generation is the single source).
mod bridges_gen;
