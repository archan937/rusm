...
[Embedding RUSM as a library](#embedding) at the end.)
...
## Write a Rust component {#rust-component}
...
## Write a TypeScript component {#ts-component}
...
[byte streams](#streaming) are all typed by the **`rusm-ts` package**.
...
## Serve over HTTP, WebSocket & SSE {#serve}
...
> [js-runner](#ts-component): `Process.self()`,
...
## Streaming between processes {#streaming}
...
[serving](#serve) path (SSE/WS bodies ride a byte stream under the hood) and never touch it
directly. When you genuinely need a direct process-to-process pipe, the producer/consumer
API and its real use cases are documented in
[byte streams](./concepts/byte-streams.md).
...
The app model above is how most projects use RUSM. But the runtime is also a plain Rust
...
### The OTP core, without any Wasm {#otp-core}
...
Wasm. See [links & supervision](./concepts/links-and-supervision.md).

### Hosting a compiled component {#embed-wasm}
