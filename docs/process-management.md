...
> [components & the actor world](./concepts/components-and-the-actor-world).
...
[serving](./serving) path (SSE/WS bodies ride a byte stream under the hood) and never touch it
directly. When you genuinely need a direct process-to-process pipe, the producer/consumer
API and its real use cases are documented in [byte streams](./concepts/byte-streams).
