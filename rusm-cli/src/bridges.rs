...
pub enum HostImpl {
    ...
    /// delegation shim and a Go dispatch runner (`rusm_runner.go`). The runner is compiled by
...
pub fn gen_ts_runner(bridge: &BridgeSpec) -> String {
    gen_ts_dispatch_runner(&bridge.name, &bridge.runner_name())
}

/// The TS dispatch runner body, by display `name` and `runner_name` — shared by custom
/// bridges (`bridge:<name>`) and auth hooks (`auth:<name>`), since both are a resident actor
/// that `require("./host")` and dispatches the named export over the `{fn, args, replyTo}`
/// envelope. (TS dispatch is dynamic — no WIT needed — so one runner serves both.)
pub fn gen_ts_dispatch_runner(name: &str, runner_name: &str) -> String {
    format!(
        "// GENERATED by `rusm build` — do not edit.\n\
         // Dispatch runner for `{name}`. Receives tagged JSON requests, calls the matching\n\
         // export from host.ts, and sends back a tagged reply. Runs as a resident actor:\n\
         // \"{runner_name}\".\n\
         //\n\
         // It is a **worker** (`export default async`): the js-runner's entry awaits it, so the\n\
         // loop stays alive — and, unlike a top-level loop, it bundles as CommonJS (no top-level\n\
         // await) and runs inside the runner's async context.\n\
         ...
         \x20 Process.register(\"{runner_name}\");\n\
         \x20 Process.setLabel(\"{runner_name}\");\n\
         \x20 while (true) {{\n\
         \x20   const raw = await Process.receiveText();\n\
         \x20   let msg: {{ fn: string; args: unknown[]; context?: Record<string, string>; replyTo: {{ pid: string; callId: string }} }};\n\
         \x20   try {{\n\
         ...
         \x20   // The caller's host-only claims context (forwarded in-band by the shim), exposed to\n\
         \x20   // host code via rusm-ts `context()`. Empty for a non-bridge runner / auth call.\n\
         \x20   (globalThis as Record<string, unknown>).__rusm_bridge_context = msg.context ?? {{}};\n\
...
/// is direct — no import), and sends back a tagged reply. Written to `bridges/<name>/rusm_runner.go`;
...
pub fn gen_go_bridge_runner(bridge: &BridgeSpec) -> Result<String> {
    ...
    Ok(format!(
        "// GENERATED by `rusm build` — do not edit.\n\
         ...
         \t\t\tcontinue\n\
         \t\t}}\n\
         \t\t// The caller's host-only claims context (forwarded in-band by the shim), exposed\n\
         \t\t// to host code via rusm.Context(). Empty for a non-bridge runner.\n\
         \t\trusm.SetContext(req.Context)\n\
         \t\tresult := dispatch(req.Fn, req.Args)\n\
         \t\tresultBytes, _ := json.Marshal(result)\n\
         \t\treply := append([]byte(req.ReplyTo.CallID+\":\"), resultBytes...)\n\
         \t\tif pid, ok := rusm.ParsePid(req.ReplyTo.Pid); ok {{\n\
...
pub fn gen_bridges_module(bridges: &[&BridgeSpec], auth: &[crate::auth::AuthSpec]) -> String {
    let mut out = String::from(
    ...
    // Mount each Rust auth hook's `host.rs` (TS/Go hooks delegate via a resident runner).
    out.push_str(&crate::auth::gen_auth_mounts(auth));
    // The linker param is unused when there are no bridges (an auth-hook-only app), so name it
    // `_linker` then to keep the generated crate warning-clean.
    let linker_param = if bridges.is_empty() {
        "_linker"
    } else {
        "linker"
    };
    out.push_str(&format!(
        "/// Register every custom application bridge into the component linker.\n\
         pub fn extend({linker_param}: &mut rusm_wasm::BridgeLinker) -> rusm_wasm::wasmtime::Result<()> {{\n",
    ));
    for bridge in bridges {
        out.push_str(&format!(
    ...
    // `init` is emitted when there are TS/Go bridge runners to boot **or** any auth hooks to
    // register (both need the runtime, post-build). The generated `src/main.rs` calls it via
    // `serve_with_init`; a pure-Rust-bridge app with no auth hooks needs no `init`.
    let non_rust: Vec<&&BridgeSpec> = bridges.iter().filter(|b| !b.is_rust_host()).collect();
    if !non_rust.is_empty() || !auth.is_empty() {
        out.push_str(
            "\n/// Register the app's serving auth hooks and boot any TS/Go bridge runners as\n\
             /// resident actors. Call once after `rusm_cli::host::build_runtime` and before the\n\
             /// first component spawn — the generated `src/main.rs` does this via `serve_with_init`.\n\
        ...
        // Register each auth hook on the runtime (after any runner above is up).
        out.push_str(&crate::auth::gen_auth_registration(auth));
        out.push_str("    Ok(())\n}\n");
    }
...
pub fn generate_host_files(root: &Path) -> Result<Vec<BridgeSpec>> {
    ...
    // Auth hooks are host code too — they share the generated host crate. An app with auth
    // hooks but no bridges still needs the crate (to compile + register its Rust hooks).
    let auth = crate::auth::discover(root)?;
    ...
    for (bridge, contract) in bridges.iter().zip(contracts.iter()) {
        ...
        match &bridge.host_impl {
            ...
            HostImpl::Go(_) => {
                std::fs::write(
                    bridge.dir.join("rusm_runner.go"),
                    gen_go_bridge_runner(bridge)?,
                )?;
    ...
    // Write the dispatch runner for each TS/Go auth hook (a resident `auth:<name>` actor). A
    // Rust auth hook is compiled in directly (mounted by `gen_bridges_module`), so it has none.
    for hook in &auth {
        match &hook.host_impl {
            HostImpl::TypeScript(_) => {
                std::fs::write(
                    hook.dir.join("_runner.ts"),
                    crate::auth::gen_ts_auth_runner(&hook.name),
                )?;
            }
            HostImpl::Go(_) => {
                std::fs::write(
                    hook.dir.join("rusm_runner.go"),
                    crate::auth::gen_go_auth_runner(&hook.name),
                )?;
                let gomod = hook.dir.join("go.mod");
                if !gomod.is_file() {
                    std::fs::write(&gomod, gen_go_bridge_gomod(&format!("auth-{}", hook.name)))?;
                }
            }
            HostImpl::Rust(_) => {}
        }
    ...
    std::fs::write(
        src.join("bridges.rs"),
        gen_bridges_module(&bridge_refs, &auth),
    )?;

    // Write the generated `main.rs` when the app needs `init` (TS/Go runners to boot, or auth
    // hooks to register) and the author hasn't supplied one — both call `serve_with_init`. A
    // pure-Rust-bridge app with no auth hooks keeps its own `host::serve` `main.rs`.
    let main_rs = src.join("main.rs");
    let needs_init = needs_delegation || !auth.is_empty();
    if needs_init && !main_rs.exists() {
        std::fs::write(&main_rs, MAIN_RS_BRIDGE_RUNNERS)?;
    } else if !auth.is_empty() && main_rs.exists() {
        // An author-written `main.rs` must run `init` to register the auth hooks; a plain
        // `host::serve` would silently skip them (then `serve` fails resolving the listener's
        // `authentication`). Fail loudly here with the fix, rather than ship an unregistered hook.
        let contents = std::fs::read_to_string(&main_rs).unwrap_or_default();
        if !contents.contains("serve_with_init") {
            bail!(
                "src/main.rs calls `host::serve`, which does not register auth hooks. Auth hooks \
                 need `init`: call `rusm_cli::host::serve_with_init(root, &cfg, bridges::extend, \
                 bridges::init)`, or delete src/main.rs to use the generated one."
            );
        }
    }
...
mod tests {
    ...
    fn gen_ts_runner_generates_dispatch_loop() {
        ...
        // Installs the forwarded claims context per call, so host.ts reads it via `context()`.
        assert!(
            runner.contains("__rusm_bridge_context = msg.context"),
            "context installed per call: {runner}"
        );
    }
    ...
    fn gen_go_bridge_runner_generates_dispatch_loop() {
        ...
        assert!(
            runner.contains("rusm.ReceiveBytes()"),
            "receive loop: {runner}"
        );
        assert!(
        ...
        assert!(
            runner.contains("rusm.ParsePid(req.ReplyTo.Pid)")
                && runner.contains("rusm.SendBytes(pid, reply)"),
            "sends reply via SendBytes(ParsePid(...)): {runner}"
        );
        // Installs the forwarded claims context per call, so host.go reads it via rusm.Context().
        assert!(
            runner.contains("Context map[string]string `json:\"context\"`")
                && runner.contains("rusm.SetContext(req.Context)"),
            "context installed per call: {runner}"
        );
    }
    ...
    fn generate_host_files_writes_go_runner_and_creates_go_mod_when_absent() {
        ...
        assert!(
            root.join("bridges/weather/rusm_runner.go").is_file(),
    ...
    #[test]
    fn generate_host_files_wires_a_rust_auth_hook_into_the_host_crate() {
        // A Rust auth hook is host code: `generate_host_files` mounts its `host.rs` and emits
        // an `init` that registers it on the runtime — even with no custom bridges, the app
        // still gets the generated crate (its hook must compile in). The generated `main.rs`
        // calls `serve_with_init` so `init` (and thus the registration) actually runs.
        let root = app_dir("gen-auth");
        std::fs::create_dir_all(root.join("auth/jwt")).unwrap();
        std::fs::write(
            root.join("auth/jwt/host.rs"),
            "pub async fn authenticate(_req: rusm_wasm::AuthRequest) -> rusm_wasm::AuthVerdict {\n\
             \x20   rusm_wasm::AuthVerdict::Deny\n}\n",
        )
        .unwrap();

        generate_host_files(&root).unwrap();

        let generated = std::fs::read_to_string(root.join("src/bridges.rs")).unwrap();
        assert!(
            generated.contains("#[path = \"../auth/jwt/host.rs\"]")
                && generated.contains("pub mod auth_jwt;"),
            "the Rust auth hook is mounted: {generated}"
        );
        assert!(
            generated.contains("pub fn init(")
                && generated.contains("wasm.register_auth_hook(")
                && generated.contains("rusm_wasm::auth_hook(auth_jwt::authenticate)"),
            "init registers the hook on the runtime: {generated}"
        );
        assert!(
            root.join("src/main.rs").exists(),
            "an auth-hook app gets the generated serve_with_init main.rs"
        );
    }

    #[test]
    fn an_author_main_rs_without_init_plus_auth_hooks_fails_loudly() {
        // Guard: a hand-written src/main.rs that calls `host::serve` (no `init`) can't register
        // auth hooks. Adding an auth hook to such an app must fail at build with the fix — never
        // ship a hook the operator wrote but the binary never registers.
        let root = app_dir("auth-main-conflict");
        std::fs::create_dir_all(root.join("auth/jwt")).unwrap();
        std::fs::write(
            root.join("auth/jwt/host.rs"),
        ...
        .unwrap();
        // An author-written main.rs that uses `host::serve` (no `init`) — the conflict.
        std::fs::create_dir_all(root.join("src")).unwrap();
        std::fs::write(
            root.join("src/main.rs"),
            "fn main() { rusm_cli::host::serve(p, &cfg, bridges::extend); }",
        )
        .unwrap();

        let err = generate_host_files(&root).unwrap_err().to_string();
        assert!(
            err.contains("serve_with_init") && err.contains("does not register auth hooks"),
            "the conflict is reported with the fix: {err}"
        );
    }
